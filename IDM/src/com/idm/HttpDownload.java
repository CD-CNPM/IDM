...
public class HttpDownload extends Download {
	...
	public void run() {
		...
		try {
			...
			//kiểm tra response code, nếu không phải mã code 2xx thì báo lỗi (mã code 2xx là request thành công)
...
//						DownloadThread downloadThread = new HttpDownloadThread(1, URL, FileUtil.joinPath(outputFolder, dFileName), startByte, endByte);
//						dDownloadThreadList.add(downloadThread);
						...
						for (int i = 1; endByte < dFileSize; i++) {
							...
							downloadThread = new HttpDownloadThread(i, dURL, FileUtil.joinPath(dOutputFolder, dFileName), startByte, endByte);
							dDownloadThreadList.add(downloadThread);
						}
					}
					// If file size smaller than 400KB or not support resume, use one thread
					else {
						HttpDownloadThread downloadThread = new HttpDownloadThread(1, dURL, FileUtil.joinPath(dOutputFolder, dFileName), 0, (int) dFileSize);
						dDownloadThreadList.add(downloadThread);
					}
				}
